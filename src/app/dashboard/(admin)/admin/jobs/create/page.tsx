...
// MOCK_USERS removed - using real DB users from useUserStore
...
// ALL_EMPLOYEES is now derived from userStore (real DB data) inside the component
...
export default function CreateJobPage() {
  ...
  const dbUsers = useUserStore((state) => state.users);
  ...
  const isLeadTechnician = currentUser?.role && (currentUser.role === 'lead_technician' || currentUser.role.startsWith('lead_'));
  ...
  // Lead technician - ดึงจาก DB จริง ไม่ใช่ Mock
  ...
  // Employee - ดึงจาก DB จริง กรองเฉพาะที่ยังว่างอยู่
  const availableEmployees: Employee[] = React.useMemo(() => {
    return dbUsers
      .filter(u => u.role === 'employee' && isEmployeeAvailable(u.id))
      .map(u => ({ value: u.id, label: u.name }));
  }, [dbUsers, isEmployeeAvailable]);
  ...
  React.useEffect(() => {
    const savedSignature = getSignature(SIGNATURE_STORAGE_KEY);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // ✅ Validation: เช็คชื่อใบงานซ้ำ (เฉพาะใบงานที่ยังไม่ได้ยกเลิกหรือเสร็จสิ้น)
    const isDuplicateTitle = jobs.some(j => 
    ...
    if (isDuplicateTitle) {
      setErrorMessage(`มีใบงานชื่อ "${title}" กำลังดำเนินการอยู่ในระบบแล้ว กรุณาใช้ชื่ออื่นหรือตรวจสอบใบงานเดิม`);
      setIsErrorAlertOpen(true);
      return;
    }
    ...
    try {
      ...
      const newJobId = await createJob(jobDataWithAttachments);
      if (!newJobId) {
        setErrorMessage("ไม่สามารถสร้างใบงานได้ กรุณาลองใหม่อีกครั้ง");
        setIsErrorAlertOpen(true);
        setIsSubmitting(false);
        return;
      }
