  ...
  return (
    <div className="p-3 sm:p-6">
      ...
      <div className="mb-4 sm:mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-1 sm:mb-2">Inventory Management</h1>
        <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">หน้าจัดการอุปกรณ์</p>
      </div>
          ...
          <div className="block sm:hidden">
            <Select value={statusFilter} onValueChange={(value) => {
              ...
              <SelectContent className="bg-white dark:bg-[#1a1d29] border-gray-200 dark:border-gray-800">
                <SelectItem value="all" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">All Status</SelectItem>
                <SelectItem value="Available" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Available</SelectItem>
                <SelectItem value="In Use" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">In Use</SelectItem>
                <SelectItem value="Maintenance" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Maintenance</SelectItem>
                <SelectItem value="Out of Stock" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Out of Stock</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700/30 bg-gray-50 dark:bg-transparent">
                ...
                <th className="px-6 py-4 text-center text-sm font-semibold text-gray-700 dark:text-gray-300">Quantity</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Requite From</th>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{item.requiteFrom}</td>
                    <td className="px-6 py-4">
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-gray-500 dark:text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-lg font-medium text-gray-600 dark:text-gray-400">ไม่พบข้อมูลอุปกรณ์</p>
                      <p className="text-sm text-gray-500 dark:text-gray-500">ลองค้นหาด้วยคำอื่นหรือเพิ่มอุปกรณ์ใหม่</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      <div className="md:hidden space-y-3">
        {currentItems.length > 0 ? (
          currentItems.map((item) => (
            <div key={item.id} className="bg-white dark:bg-transparent rounded-xl border border-gray-200 dark:border-gray-700/30 p-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white font-semibold">{item.quantity}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-gray-600 dark:text-gray-400 mb-1">Requite From</p>
                  <p className="text-gray-900 dark:text-white">{item.requiteFrom}</p>
                </div>
              </div>
            </div>
          ))
          ...
          <div className="bg-white dark:bg-[#1a1d29] rounded-xl border border-gray-200 dark:border-gray-800 p-8 text-center">
            ...
            <p className="text-base font-medium text-gray-600 dark:text-gray-400 mb-1">ไม่พบข้อมูลอุปกรณ์</p>
            <p className="text-sm text-gray-500 dark:text-gray-500">ลองค้นหาด้วยคำอื่นหรือเพิ่มอุปกรณ์ใหม่</p>
          </div>
        )}
      </div>
      ...
      {filteredItems.length > 0 && (
        <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4">
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 order-2 sm:order-1">
            แสดง <span className="font-medium text-gray-900 dark:text-white">{indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredItems.length)}</span> จาก <span className="font-medium text-gray-900 dark:text-white">{filteredItems.length}</span> รายการ
          </p>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-[550px] w-[calc(100%-2rem)] max-h-[85vh] overflow-y-auto bg-white dark:bg-[#1a1d29] border-gray-200 dark:border-gray-800 z-50 fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl text-gray-900 dark:text-white">{editingItem ? 'แก้ไขอุปกรณ์' : 'เพิ่มอุปกรณ์ใหม่'}</DialogTitle>
          ...
          <div className="grid gap-4 sm:gap-5 py-4">
            <div className="grid gap-2">
              <Label htmlFor="title" className="text-sm font-medium text-gray-700 dark:text-gray-300">Title <span className="text-red-500">*</span></Label>
              <Input
                id="title"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="grid gap-2 w-full">
                <Label htmlFor="status" className="text-sm font-medium text-gray-700 dark:text-gray-300">Status <span className="text-red-500">*</span></Label>
              ...
              <div className="grid gap-2 w-full">
                <Label htmlFor="type" className="text-sm font-medium text-gray-700 dark:text-gray-300">Type <span className="text-red-500">*</span></Label>
                <Select value={formData.type} onValueChange={(value) => setFormData({ ...formData, type: value })}>
                  <SelectTrigger className="bg-gray-50 dark:bg-[#0f1117] border-gray-200 dark:border-gray-800 text-gray-900 dark:text-white focus:border-blue-500 dark:focus:border-blue-600 w-full">
                    <SelectValue placeholder="เลือกประเภท" />
                  </SelectTrigger>
                  <SelectContent className="bg-white dark:bg-[#1a1d29] border-gray-200 dark:border-gray-800">
                    <SelectItem value="Laptop" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Laptop</SelectItem>
                    <SelectItem value="Monitor" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Monitor</SelectItem>
                    <SelectItem value="Peripheral" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Peripheral</SelectItem>
                    <SelectItem value="Network" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Network</SelectItem>
                    <SelectItem value="Server" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Server</SelectItem>
                    <SelectItem value="Other" className="text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-[#0f1117]">Other</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="quantity" className="text-sm font-medium text-gray-700 dark:text-gray-300">Quantity <span className="text-red-500">*</span></Label>
              <Input
                id="quantity"
                type="number"
            ...
            <div className="grid gap-2">
              <Label htmlFor="requiteFrom" className="text-sm font-medium text-gray-700 dark:text-gray-300">Requite From <span className="text-red-500">*</span></Label>
              <Input
                id="requiteFrom"
